...
library(ggpubr)  # for correlation between variables
...
# show data with the first 5 rows
data %>% head(5)
```
...
```{r, echo=FALSE}
# plot distribution in electric range
...
fig.plug <- plot_ly(count.make.plugin, labels = ~ make, values = ~  Observations.Make.Plugin, type = 'pie',
              textposition = 'inside',  textinfo = 'label+percent', insidetextfont = list(color = '#FFFFFF'),                  hoverinfo = 'text', text = ~paste('$', make,  Observations.Make.Plugin),
              marker = list(colors = colors, line = list(color = '#FFFFFF', width = 1)), showlegend = FALSE)
